...
export async function getUserBadges(userId: string): Promise<BadgeType[]> {
  try {
    ...
    // Special handling for Pioneer badge - check eligibility but don't auto-award
    // Auto-awarding is now handled by checkAndAwardBadges to prevent duplicates
    const isPioneerEligible = await checkPioneerEligibility(userId)
    if (isPioneerEligible && !userBadges.includes('PIONEER')) {
      // Only return the badge if it's already been awarded, don't auto-award here
      console.log(`[BADGE SERVICE] User ${userId} is eligible for Pioneer badge but hasn't been awarded yet`)
    }
...
export async function awardBadge(userId: string, badgeType: BadgeType): Promise<boolean> {
  try {
    // First check if user already has this badge in Redis
    const allBadges = await getAllUserBadges()
    const existingUser = allBadges.find(u => u.userId === userId)
    
    if (existingUser && existingUser.badges.includes(badgeType)) {
      console.log(`[BADGE SERVICE] User ${userId} already has badge ${badgeType}, skipping award`)
    ...
    if (!userExists) {
      console.error(`[BADGE SERVICE] User ${userId} not found in database, cannot award badge`)
      return false
    }
    ...
    console.log(`[BADGE SERVICE] Successfully awarded badge ${badgeType} to user ${userId}`)
